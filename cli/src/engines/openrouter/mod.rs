...
fn read_key(config: &Config) -> Result<String, EngineFailure> {
    ...
    match std::fs::read_to_string(path) {
        ...
        _ => Err(EngineFailure::Unavailable(
            "Cloud LLM has no key. Store one: printf '%s' \"$KEY\" | grammachy setup --openrouter-key. (reason: no_key)"
                .to_string(),
        )),
    }
}
...
fn classify_status(status: u16, body: &str) -> EngineFailure {
    ...
    match status {
        401 | 403 => EngineFailure::Unavailable(
            "OpenRouter rejected the key. Store a new one: printf '%s' \"$KEY\" | grammachy setup --openrouter-key. (reason: rejected_key)"
                .to_string(),
        ),
