...
//! `.part` file: a resumed install is minutes rather than the whole archive
//! again, and the whole point of Cancel is that it is cheap to change one's
//! mind.
//!
...
//! set a flag; [`super::transfer::curl`] polls that flag, kills its child, and
...
///
/// Only an install calls this: every other verb finishes in milliseconds and
