...
The Local LLM engine needs a weights file of several gigabytes.
Until now the only way to get one was `grammachy setup` in a terminal.
...
The CLI prints nothing at all while `curl` runs.
The shell runs `grammachy model list` once a second and reads the length of the `.part` file.
The bar is that length against the pinned byte size of the row.

Cancel is a SIGTERM to the running process.
The CLI installs a handler that only sets a flag.
The transfer loop reads that flag, kills its `curl` child, and exits 1 with code `cancelled`.
...
- **A progress line on stdout or stderr, read as it arrives.**
  It would give a smoother bar.
  It also breaks the one promise every subcommand makes: one JSON object on stdout, and stderr for logs only.
  A shell that parses two kinds of output on one stream has to guess which it is reading.
- **The download in QML, through Quickshell.**
  There is no HTTP client in the plugin.
  Adding TLS to the shell to fetch a file the CLI already fetches puts the pinned digest in two places.
...
- Every catalogue row is pinned twice, by sha256 and by byte size.
  The digest is what the rename checks.
  The size is what the free-space check and the bar measure against.
