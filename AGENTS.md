...
- Every producer the plugin reads is bounded before the read: `MAX_STDIN_BYTES` in `cli/src/main.rs`, `MAX_RESPONSE_BYTES` and the match caps in the `languagetool` adapter, and the `wl-paste` commands of `ui/capture.js`.
